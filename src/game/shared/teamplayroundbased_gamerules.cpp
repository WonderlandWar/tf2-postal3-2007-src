...
void CTeamplayRoundBasedRules::CheckWaitingForPlayers( void )
{
	...
	if( m_bInWaitingForPlayers )
	{
		...
		if( gpGlobals->curtime > m_flWaitingForPlayersTimeEnds && m_flRestartRoundTime < 0 && !m_bAwaitingReadyRestart )
		{
			...
			CTeamControlPointMaster *pMaster = dynamic_cast<CTeamControlPointMaster*>( gEntList.FindEntityByClassname( NULL, "team_control_point_master" ) );
			if ( pMaster && pMaster->PlayingMiniRounds() && pMaster->GetCurrentRound() )
			{
				SetRoundToPlayNext( pMaster->GetCurrentRound()->GetEntityName() );
			}
		}
		else
		{
...
void CTeamplayRoundBasedRules::State_Think_STALEMATE( void )
{
	...
	if ( iDeadTeam && iAliveTeam )
	{
		...
		if ( !m_bForceMapReset )
		{
			...
			while ( ( pTempEnt = gEntList.FindEntityByClassname( pTempEnt, "team_control_point_master" ) ) != NULL )
			{
				variant_t sVariant;
				sVariant.SetInt( iAliveTeam );
				pTempEnt->AcceptInput( "SetWinnerAndForceCaps", NULL, NULL, sVariant, 0 );
				bMasterHandled = true;
			}
...
void CTeamplayRoundBasedRules::SetWinningTeam( int team, int iWinReason, bool bForceMapReset /* = true */, bool bSwitchTeams /* = false*/ )
{
	...
	// only reward the team if they have won the map and we're going to do a full reset and we're changing maps
	if ( bForceMapReset )
	{
...
//			round timer expired 
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::SetStalemate( bool bForceMapReset )
{
	SetForceMapReset( bForceMapReset );
