...
const char *CTFWeaponBase::GetViewModel( int iViewModel ) const
{
	...
	KeyValues *pKV = GetTFWpnData().m_pClassViewModelKV;
	if ( pKV )
	{
		return pKV->GetString( pData->m_szClassName, NULL );
	}
...
void CWeaponInvisProxy::OnBind( C_BaseEntity *pEnt )
{
	...
	const float flPercentInvisible = pPlayer->GetPercentInvisible();

	if ( pPlayer->IsEnemyPlayer() && flPercentInvisible > tf_teammate_max_invis.GetFloat() )
	{
		m_pPercentInvisible->SetFloatValue( tf_teammate_max_invis.GetFloat() );
	}
	else
	{
		m_pPercentInvisible->SetFloatValue( flPercentInvisible );
	}
}
