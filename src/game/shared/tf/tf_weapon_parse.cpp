...
void CTFWeaponInfo::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	// Class specific viewmodels
	for ( int i = 1; i < TF_CLASS_COUNT_ALL; ++i )
	{
		char buf[128];
		TFPlayerClassData_t *pData = GetPlayerClassData( i );
		V_snprintf( buf, 128, "ViewModel_%s", pData->m_szClassName );
		const char *pszKV = pKeyValuesData->GetString( buf, NULL );
		if ( pszKV )
		{
			if ( !m_pClassViewModelKV )
			{
				m_pClassViewModelKV = new KeyValues( "class_viewmodels" );
			}
			m_pClassViewModelKV->SetString( pData->m_szClassName, pszKV );
		}
	}

	m_bDontDrop = ( pKeyValuesData->GetInt( "DontDrop" ) > 0 );
}
