...
#include "ammodef.h"
	...
	ConVar  tf_flamethrower_velocity( "tf_flamethrower_velocity", "1700.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY, "Initial velocity of flame damage entities." );
...
void CTFFlameThrower::PrimaryAttack()
{
	...
	UTIL_TraceLine( vecEye, vecMuzzlePos, MASK_SOLID, this, 0, &trace );
...
IMPLEMENT_NETWORKCLASS_ALIASED( TFFlameRocket, DT_FlameRocket )
BEGIN_NETWORK_TABLE( CTFFlameRocket, DT_FlameRocket )
END_NETWORK_TABLE()
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFlameRocket *CTFFlameRocket::Create( const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner )
{
	CTFFlameRocket *pRocket = static_cast<CTFFlameRocket*>( CTFBaseRocket::Create( "tf_weapon_flamerocket", vecOrigin, vecAngles, pOwner ) );
