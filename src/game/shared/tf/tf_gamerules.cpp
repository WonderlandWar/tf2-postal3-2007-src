...
{	
	...
	{
		variant_t emptyVariant;
		pTempEnt->AcceptInput( "RoundStart", NULL, NULL, emptyVariant, 0 );
	}
...
void CTFGameRules::SendWinPanelInfo( void )
{
	...
	if ( winEvent )
	{
		...
		if ( !bRoundComplete && ( TEAM_UNASSIGNED != m_iWinningTeam ) )
		{
			...
			if ( pMaster )
			{
				winEvent->SetInt( "rounds_remaining", pMaster->CalcNumRoundsRemaining( m_iWinningTeam ) );
			}
		}
...
void CTFGameRules::SetupSpawnPointsForRound( void )
{
	...
	CTeamControlPointRound *pCurrentRound = pMaster->GetCurrentRound();
	if ( !pCurrentRound )
	{
		return;
	}
