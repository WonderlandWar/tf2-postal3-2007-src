...
BEGIN_NETWORK_TABLE( CWeaponMedigun, DT_WeaponMedigun )
	...
	SendPropArray3( SENDINFO_ARRAY3(m_hHealingTargets), SendPropEHandle( SENDINFO_ARRAY(m_hHealingTargets) ) ),
	...
	//RecvPropArray3( RECVINFO_ARRAY(m_hHealingTargets), RecvPropEHandle( RECVINFO( m_hHealingTargets[0] ) ), 0, RecvProxy_HealingTargets ),
...
BEGIN_PREDICTION_DATA( CWeaponMedigun  )
	...
	DEFINE_PRED_ARRAY( m_hHealingTargets, FIELD_EHANDLE, MAX_PLAYERS, FTYPEDESC_INSENDTABLE ),
...
bool CWeaponMedigun::HealingTarget( CBaseEntity *pTarget )
{
	...
	{
		if ( pTarget == m_hHealingTargets[i].Get() )
			return true;
	}
...
void CWeaponMedigun::RemoveHealingTarget( int iSlot, bool bStopHealingSelf )
{
	...
	if ( iSlot == -1 || !iSlot )
	{
		...
		{
			// HACK: For now, just deal with players
			if ( m_hHealingTargets[0]->IsPlayer() )
			{
				CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
...
void CWeaponMedigun::UpdateEffects( void )
{
	...
	for ( int i = 0; i < m_hHealingTargetEffects.Count(); ++i )
	{
		m_hHealingTargetEffects[i].pTarget = NULL;
		m_hHealingTargetEffects[i].pEffect = NULL;
	}
	...
	// TFP3: Next bit might be inaccurate...
