...
#if 0 // TFP3: Testing the Homing Rockets
	Vector vecDirection;
	AngleVectors( pPlayer->EyeAngles(), &vecDirection );

	trace_t tr;
	UTIL_TraceLine( pPlayer->EyePosition(), pPlayer->EyePosition() + ( vecDirection * 2000 ), MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );

	CTFProjectile_Rocket *pProjectile = CTFProjectile_Rocket::Create( vecSrc, angForward, pPlayer, pPlayer, tr.DidHitWorld() ? NULL : tr.m_pEnt );
#else
...
CBaseEntity *CTFWeaponBaseGun::FireNail( CTFPlayer *pPlayer, int iSpecificNail )
{
	...
	{
	case TF_PROJECTILE_NAIL:
		pProjectile = CTFProjectile_Nail::Create( vecSrc, angForward, pPlayer, pPlayer );
		break;
		
	case TF_PROJECTILE_DART:
		pProjectile = CTFProjectile_Dart::Create( vecSrc, angForward, pPlayer );
		break;
