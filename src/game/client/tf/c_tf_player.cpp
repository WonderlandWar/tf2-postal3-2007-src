...
void CSpyInvisProxy::OnBind( C_BaseEntity *pEnt )
{
	...
	const float flPercentVisible = pPlayer->GetPercentInvisible();
	float flAmount;

	if ( !pPlayer->IsEnemyPlayer() )
	{
		flAmount = tf_teammate_max_invis.GetFloat();
		if ( flPercentVisible <= flAmount )
			flAmount = flPercentVisible;
	}
	else
	{
		flAmount = flPercentVisible;
	...
	m_pPercentInvisible->SetFloatValue( flAmount );
#if 0
	float r, g, b;
	...
	m_pCloakColorTint->SetVecValue( r, 0.5 );
#endif
}
...
public:
	void OnBind( void *pC_BaseEntity )
	{
		...
		if ( pPlayer )
		{
			if ( pPlayer->m_Shared.InCond( TF_COND_INVULNERABLE ) && pPlayer->m_Shared.IsInvulnerableFading() )
			{
...
void C_TFPlayer::ClientThink()
{
		...
		!( pLocalPlayer->GetObserverMode() == OBS_MODE_IN_EYE && pLocalPlayer->GetObserverTarget() == this ) )	// not if we're spectating this player first person
	{
		if ( !m_pCigaretteSmoke )
		{
			...
			m_pCigaretteSmoke = ParticleProp()->Create( "cig_smoke", PATTACH_POINT_FOLLOW, iSmokeAttachment );
		}
	}
	else	// stop the smoke otherwise if its active
	{
		if ( m_pCigaretteSmoke )
		{
			m_pCigaretteSmoke->StopEmission( false, true );
			m_pCigaretteSmoke = NULL;
		}
	}
...
//			*pCmd - 
//-----------------------------------------------------------------------------
void C_TFPlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{
