...
IMPLEMENT_SERVERCLASS_ST( CObjectSentrygun, DT_ObjectSentrygun )
	...
	SendPropInt( SENDINFO(m_iMaxAmmoShells), 9 ),
...
//-----------------------------------------------------------------------------
// If this is a better target
//-----------------------------------------------------------------------------
bool CObjectSentrygun::IsBetterTarget( CBaseEntity *pNewTarget )
{
	if ( !m_hEnemy )
		return true;
...
bool CObjectSentrygun::FindTarget()
{
	...
	CEntitySphereQuery sphere( vecOrigin, 1100, FL_OBJECT | FL_CLIENT );
	
	CBaseEntity *CurrentEntity = sphere.GetCurrentEntity();
	if (!CurrentEntity)
		return false;
	while ( !CObjectSentrygun::ValidTarget( CurrentEntity )
		|| !CObjectSentrygun::IsBetterTarget( CurrentEntity ) )
	{
		sphere.NextEntity();
		CurrentEntity = sphere.GetCurrentEntity();
		if ( !CurrentEntity )
			return false;
	}

	FoundTarget( CurrentEntity );
	return true;
}

bool CObjectSentrygun::ValidTarget( CBaseEntity *pTarget )
{
	if( !pTarget || !pTarget->IsPlayer() && !pTarget->IsBaseObject() || !pTarget->IsAlive() )
		return false;
	...
	if ( ( GetWaterLevel() == 0 && pTarget->GetWaterLevel() >= 3 ) || ( GetWaterLevel() == 3 && pTarget->GetWaterLevel() <= 0 ) )
		return false;

	// Ray trace.
	return FVisible( pTarget, MASK_SHOT | CONTENTS_GRATE );
}

//-----------------------------------------------------------------------------
// Found a Target
//-----------------------------------------------------------------------------
void CObjectSentrygun::FoundTarget( CBaseEntity *pNewTarget )
{
	m_hEnemy = pNewTarget;
	...
	if ( ( m_iAmmoShells > 0 ) || ( m_iAmmoRockets > 0 && m_iUpgradeLevel == 3 ) )
	{
		...
		if ( pNewTarget->IsPlayer() )
		{
			CTFPlayer *pPlayer = ToTFPlayer( pNewTarget );
...
void CObjectSentrygun::Attack()
{
	...
	if ( IsDisabled() || !m_hEnemy || !ValidTarget( m_hEnemy ) )
	{
		m_iState.Set( SENTRY_STATE_SEARCHING );
		m_hEnemy = NULL;
		return;
...
bool CObjectSentrygun::Fire()
{
		...
		m_flNextRocketAttack < gpGlobals->curtime )
	{
		...
		CTFProjectile_SentryRocket *pProjectile = CTFProjectile_SentryRocket::Create( vecSrc, angAimDir, this, NULL );
		if ( pProjectile )
		{
			pProjectile->SetScorer( GetBuilder() );
...
void CObjectSentrygun::Killed( const CTakeDamageInfo &info )
{
	...
	if ( pPlayer )
	{
		if ( pPlayer->GetMaxSentryKills() < GetKills() )
			pPlayer->SetMaxSentryKills( GetKills() );
	}
