...
void CTFPlayer::SetModelIndex( int index )
{
	m_nPlayerModelIndex = index;
	BaseClass::SetModelIndex( index );
}
...
CPlayerStateInfo *CTFPlayer::StateLookupInfo( int nState )
{
	...
	static CPlayerStateInfo playerStateInfos[] =
	{
		...
		{ TF_STATE_WELCOME,				"TF_STATE_WELCOME",				&CTFPlayer::StateEnterWELCOME,				NULL,	NULL },
...
void CTFPlayer::UpdateExpression( void )
{
	...
	AI_Response * const result = SpeakConcept( MP_CONCEPT_PLAYER_EXPRESSION );
	if ( !result )
	{
		ClearExpression();
...
void CTFPlayer::SpeakWeaponFire( int iCustomConcept )
{
	...
	char szScene[ MAX_PATH ];
	AI_Response * const result = SpeakConcept( iCustomConcept );
	if ( !result )
		return;
