...
void CBaseObject::Explode( void )
{
	...
	int flags = TE_EXPLFLAG_NODLIGHTS;
	...
	te->Explosion( filter, 0.0, &origin, g_sModelIndexFireball, 5.4, 15, flags, 256, 200 );
...
void CBaseObject::Killed( const CTakeDamageInfo &info )
{
	...
	if ( pScorer && pInflictor != this )
	{
		...
		const char *killer_weapon_name = TFGameRules()->GetKillingWeaponName( pInflictor, pScorer );
